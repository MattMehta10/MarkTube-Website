...
const Workspace = () => {
 ...
 const watchNext =[...(Libdata.toWatch || [])]
  ...
  return (
    <div className="flex h-full text-white font-[gilroy]">
      {/* Sidebar */}
      <aside className="w-56 border-r border-gray-50/12 bg-transparent p-6">
        <h2 className="text-2xl mb-8 font-extrabold text-white tracking-wide">Workspace</h2>
          {['Overview', 'Library', 'Settings'].map((tab) => (
            <button
              key={tab}
              className={`w-full text-left py-2 px-4 rounded-2xl border-gray-50/12 transition-all duration-200 font-bold tracking-wide
      ...
      {/* Main Content */}
      <main className="flex-1 py-5 px-10 bg-[#030712] rounded-l-3xl shadow-2xl">
        {/* Header */}
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-3xl font-extrabold text-white tracking-wide drop-shadow-lg">{activeTab.toUpperCase()}</h2>
