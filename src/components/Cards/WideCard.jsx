...
const WideCard = ({imgl,size,data}) => {
  const [green, setgreen] = useState(false);
  const [yellow, setyellow] = useState(false);
  const [red, setred] = useState(false);
  const [Cardsize, setCardsize] = useState(size);
   useEffect(() => {
    setCardsize(size); // ✅ Update only when size prop changes
  }, [size]);
...
useGSAP(() => {
  ...
  return (
    <div key={1} 
    className={ `[relative scale-${size} bg-[#ffffff] w-160 duration-75 ease-in-out rounded-2xl p-5 h-120 group]`}>
